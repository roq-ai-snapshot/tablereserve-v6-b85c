...
function ReservationListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ReservationInterface[]>(
    ...
    () =>
      getReservations({
        relations: ['user', 'restaurant', 'table_availability'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr key={record.id}>
                    ...
                    {hasAccess('table_availability', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/table-availabilities/view/${record.table_availability?.id}`}>
                          {record.table_availability?.restaurant_id}
                        </Link>
                      </Td>
                    )}
